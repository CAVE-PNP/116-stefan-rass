...
locale TM_Encoding = (* TODO fix bool this early? *)
  fixes enc :: "(nat, bool) TM \<Rightarrow> bool list"
    and is_valid_enc :: "bool list \<Rightarrow> bool"
    and dec :: "bool list \<Rightarrow> (nat, bool) TM"
  assumes valid_enc: "\<And>M. is_valid_enc (enc M)"
    and inj_enc: "inj enc"  (* TODO is this possible? the transition table is defined as a function. similar for \<open>dec (enc M) = M\<close> *)
    and enc_dec:   "\<And>M. dec (enc M) = M"
    and dec_enc: "\<And>x. is_valid_enc x \<Longrightarrow> enc (dec x) = x" (* this should be easy to achieve *)
    and invalid_rejects: "\<And>x. \<not> is_valid_enc x \<Longrightarrow> TM.rejects (dec x) w" (* a nicer version of: "\<exists>q\<^sub>0 s. dec\<^sub>U x = (rejecting_TM q\<^sub>0 s)" *)
...
definition enc_pad :: "binTM \<Rightarrow> bool list"
  where "enc_pad M = add_exp_pad (add_al_prefix (enc M))"

definition dec_pad :: "bool list \<Rightarrow> binTM"
  where "dec_pad w = dec (strip_al_prefix (strip_exp_pad w))"

lemma TM_codec_pad: "dec_pad (enc_pad M) = M"
  unfolding dec_pad_def enc_pad_def
  unfolding exp_pad_correct[OF add_alp_min] alp_correct enc_dec ..

lemma wf_TM_has_enc: "\<exists>w. dec_pad w = M"
...
theorem TM_inf_encs: "infinite {w. dec_pad w = M}"
proof (intro infinite_lists allI bexI CollectI)
  ...
  fix l
  define w' where w': "w' = (enc M) @ False # True \<up> l"
  define w where w: "w = add_exp_pad w'"

  show "l \<le> length w" unfolding w w' by force

  have "dec_pad w = dec (strip_al_prefix w')" unfolding w w' dec_pad_def
    by (subst exp_pad_correct) blast+
  also have "... = dec (enc M)" unfolding w' strip_alp_altdef ..
  also have "... = M" by (fact enc_dec)
  finally show "dec_pad w = M" .
qed
...
theorem num_equivalent_encodings:
  ...
  assumes "l > 0"
    and "dec_pad w = M"
  shows "2^(l - clog l) \<le> card {w. length w = l \<and> dec_pad w = M}" (is "?lhs \<le> card ?A")
proof -
  ...
  also have "... \<le> card {w. length w = l \<and> dec_pad w = M}"
  proof (intro card_mono)
    show "finite {w. length w = l \<and> dec_pad w = M}" using finite_bin_len_eq by simp
    show "{pad @ w' | pad. length pad = l - clog l} \<subseteq> {w. length w = l \<and> dec_pad w = M}"
    proof safe
      fix pad::bin
      ...
      show "dec_pad (pad @ w') = M" unfolding dec_pad_def strip_exp_pad_def
        unfolding lpw' Let_def h3 unfolding w'_def
        using \<open>dec_pad w = M\<close> by (fold dec_pad_def)
    qed
  qed
  finally show ?thesis .
qed
...
theorem embed_TM_in_len:
  fixes M l
  assumes min_word_len: "clog l \<ge> length (enc M) + 2" \<comment> \<open>The \<open>+2\<close> bits are required for the \<open>1\<^sup>+0\<close>-prefix.

        Note: this theorem technically also holds when the assumption @{thm min_word_len} reads
        \<^term>\<open>clog l > length (enc M) \<longleftrightarrow> clog l \<ge> length (enc M) + 1\<close>,
...
proof
  ...
  let ?\<rho>M = "enc M" let ?l\<rho> = "length ?\<rho>M"
  ...
  show "dec_pad w = M" unfolding dec_pad_def w_correct w'_correct
    by (fact enc_dec)
qed
