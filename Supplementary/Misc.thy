...
lemma of_bool_mono:
  fixes \<alpha> :: "'a :: {zero_neq_one, zero_less_one}"
  shows "(P \<longrightarrow> Q) \<longleftrightarrow> (of_bool P \<le> (of_bool Q :: 'a))"
  by (induction P; induction Q) (simp_all add: order_antisym_conv)
...
proof
  let ?B = "A - {x}"
  from assms show "card ?B = k" and "A = insert x ?B" and "x \<notin> ?B" by force+
qed
